...
public abstract class HashTable  extends Monitorable {
    ...
    protected HashTable(final int size) {
        this.table = new String[size];  //dynamic table
        this.entries = 0;               // empty
    }
    /**
    ...
    protected int hashFunction(final String key) {
        assert key.length() == KEY_LENGTH;      // checks whether key.length equals the constant declared in instance
                                                // or else throws an exception at runtime

        int hashVal = 0;

        for(int i=0; i<key.length(); i++) {
            hashVal += weights[i]*key.charAt(i); // shifting
        }
        return hashVal % this.tableSize();  // could still use table.length
    }
    /**
    ...
    public void insert(String key) {
        ...
        if (table[index]==null) { //key not found 
            table[index]= key;    // key inserted
            this.entries++;
        }
    }
